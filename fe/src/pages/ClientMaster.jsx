  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10 max-w-full">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-6 gap-4 relative">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight mb-2">Client Master</h1>
          <ol className="flex items-center space-x-2 text-sm text-slate-500 dark:text-slate-400">
            <li><a href="/" className="hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors">Home</a></li>
            <li>/</li>
            <li className="font-semibold text-slate-900 dark:text-slate-200">Client Master</li>
          </ol>
        </div>
      </div>
      ...
      <div className="bg-white/80 dark:bg-slate-950/80 backdrop-blur-md rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex-1 flex flex-col">
        <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center bg-indigo-600 dark:bg-slate-900">
           <h3 className="font-semibold text-white">Client Master List</h3>
        ...
        <div className="flex-1 overflow-auto">
          {loading ? (
             ...
             <div className="flex flex-col items-center justify-center h-64 text-slate-400 italic text-sm">
               No clients found. Add records to view them here.
             </div>
          ) : (
            <table className="w-full text-left border-collapse min-w-[1000px]">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800 text-xs tracking-wider text-slate-500 dark:text-slate-400 bg-slate-50/50 dark:bg-slate-900/50 font-bold uppercase">
                  ...
                  <th className="px-5 py-4">Client Name<br/><span className="text-[10px] font-normal text-indigo-400/80">(Client Code)</span></th>
                ...
                {clients.map((c, idx) => (
                  <tr key={c.id} className="border-b border-slate-100 dark:border-slate-800/50 hover:bg-slate-50/50 dark:hover:bg-slate-800/20 transition-colors group">
                    ...
                    <td className="px-5 py-3 text-sm font-semibold text-indigo-600 dark:text-indigo-400 leading-tight">
                       {c.name || '-'} <br/>
                       <span className="text-xs font-normal text-slate-400 dark:text-slate-500">({c.profile_code})</span>
                    </td>
                    <td className="px-5 py-3 text-sm font-medium text-slate-600 dark:text-slate-300">{c.contact_person || '-'}</td>
                    <td className="px-5 py-3 text-sm font-medium text-slate-600 dark:text-slate-300">{c.mobile_no || '-'}</td>
                    <td className="px-5 py-3 text-sm font-medium text-slate-400 dark:text-slate-500">{c.password || '-'}</td>
                    <td className="px-5 py-3 text-sm font-medium text-slate-600 dark:text-slate-300">{getCityName(c.city)}</td>
                    <td className="px-5 py-3 text-sm text-slate-500 max-w-[150px] truncate" title={c.address}>{c.address || '-'}</td>
